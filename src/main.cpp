...
IRAM_ATTR static uint8_t adpcm_encode_sample(int16_t sample, AdpcmState &s) {
...
IRAM_ATTR static int16_t adpcm_decode_sample(uint8_t code, AdpcmState &s) {
    int16_t step  = ADPCM_STEP_TABLE[s.idx];
    int32_t diffq = step >> 3;
    if (code & 4) diffq += step;
    if (code & 2) diffq += step >> 1;
...
IRAM_ATTR static void adpcm_encode_block(const int16_t *in, uint8_t *out, uint16_t samples, AdpcmState &s) {
...
IRAM_ATTR static void adpcm_decode_block(const uint8_t *in, int16_t *out, uint16_t nibbles, AdpcmState &s) {
...
IRAM_ATTR static uint16_t compute_rms(const int16_t *buf, uint16_t len) {
    if (len == 0) return 0;
    uint64_t sum = 0;
...
IRAM_ATTR static void remove_dc_offset(int16_t *buf, uint16_t len) {
    if (len == 0) return;
    int32_t sum = 0;
...
IRAM_ATTR static void apply_limiter(int16_t *buf, uint16_t len) {
    for (uint16_t i = 0; i < len; i++) {
...
IRAM_ATTR static void apply_fade_in(int16_t *buf, uint16_t len, uint8_t fadeLen) {
...
static void update_signal_bars() {
    ...
    if (elapsed >= SIGNAL_WINDOW_MS) {
        ...
        uint8_t quality  = (total > 0) ? (uint8_t)((gPktsInWindow * 100u) / total) : 0;
